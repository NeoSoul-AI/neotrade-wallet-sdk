...
|---|---|---|
| No raw-hash signing API exists; structured orders only; unknown fields rejected | `signing-gateway/src/gateway.ts` (`OrderIntentSchema` = union of the `.strict()` prediction legs and the `.strict()` spot legs; `signOrder` is the only entry point) | `signing-gateway/test/gateway.test.ts` ("structurally refuses anything that is not a well-formed order", "rejects malformed spot legs structurally") |
